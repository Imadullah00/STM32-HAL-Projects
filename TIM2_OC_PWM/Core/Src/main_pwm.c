...
void TIM2_Init(void)
{
	...
	tim2OC_PWM.Pulse = (htimer2.Init.Period *25)/100;
	...
	tim2OC_PWM.Pulse = (htimer2.Init.Period *45)/100;
	if(HAL_TIM_PWM_ConfigChannel(&htimer2, &tim2OC_PWM, TIM_CHANNEL_2) != HAL_OK )
	{
		Error_Handler();
	}

	tim2OC_PWM.Pulse = (htimer2.Init.Period *75)/100;
	if(HAL_TIM_PWM_ConfigChannel(&htimer2, &tim2OC_PWM, TIM_CHANNEL_3) != HAL_OK )
	{
		Error_Handler();
	}

	tim2OC_PWM.Pulse = (htimer2.Init.Period *95)/100;
	if(HAL_TIM_PWM_ConfigChannel(&htimer2, &tim2OC_PWM, TIM_CHANNEL_4) != HAL_OK )
	{
		Error_Handler();
	}
}
